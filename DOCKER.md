# 🐳 Backend Despachos - Docker

Guía para construir, ejecutar y desplegar la API Backend de Despachos con Docker.
...
# Ver logs
docker logs -f backend-despachos

# Detener
docker stop backend-despachos
docker rm backend-despachos
```
...
| SPRING_APPLICATION_NAME | backend-despachos | Nombre de la app |
...
### Inspeccionar imagen
```bash
docker inspect backend-despachos:latest
```
